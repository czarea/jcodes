...
public class TableExtractor {
	private static final CodesDataSource dataSource = CodesDataSource
			.getInstance();
	...
	public static Table getTable(String table) {
		...
		try {
			...
			// 获取信息
			List<String> primaryKeys = getTablePrimaryKeys(cataLog, table,
					conn);
	...
	private static List<String> getTablePrimaryKeys(String catalog,
		...
		ResultSet primaryKeyRs = conn.getMetaData()
				.getPrimaryKeys(null, catalog, table);
	...
	private static void getTableColumns(Table result, ResultSet columnResultSet,
		...
		while (columnResultSet.next()) {
			...
			if (columnRemarks == null && dataSource.getJdbcUrl()
					.contains("oracle")) {
				columnRemarks = getOracleColumnComments(result.getName(),
						columnName, conn);
			}
			...
			boolean isNullable = (DatabaseMetaData.columnNullable
			...
			Column column = new Column(result, sqlType, sqlTypeName, columnName,
